...
app.get('/api/:buyingDay/:nextBuyingDay', async (req, res) => {
  const result = [];
  const diff = calculateDiffInDays(req.params.buyingDay, req.params.nextBuyingDay);
  ...
  for (let i = 0; i < db.length; i++) {
    ...
    const stockOfBuyingDay =
      db[i].item_stock -
      calculateDiffInDays(date.toISOString().split('T')[0], req.params.buyingDay) /
        db[i].consumption_day;
    // nextBuyingDay時点での在庫数を計算
    const stockOfNextBuyingDay = stockOfBuyingDay - diff / db[i].consumption_day;

    console.log(date);
    console.log(`${db[i].item_name},stockOfBuyingDay:${stockOfBuyingDay}`);
    console.log(`${db[i].item_name},stockOfNextBuyingDay:${stockOfNextBuyingDay}`);
